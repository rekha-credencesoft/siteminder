  ...
  for (let index = 0; index < deluxeRoomPlans.length; index++) {
    ...
    for (let jindex = 0; jindex < deluxeRoomPlans[index].length; jindex++) {
      if (!deluxPlansToShow[deluxeRoomPlans[index][jindex].name]) {
        ...
        deluxPlansToShow[deluxeRoomPlans[index][jindex].name].push(
          deluxeRoomPlans[index][jindex]
        );
      } else {
        deluxPlansToShow[deluxeRoomPlans[index][jindex].name].push(
          deluxeRoomPlans[index][jindex]
  ...
  for (let index = 0; index < supremeRoomPlans.length; index++) {
    ...
    for (let jindex = 0; jindex < supremeRoomPlans[index].length; jindex++) {
      if (!supremePlansToShow[supremeRoomPlans[index][jindex].name]) {
        ...
        supremePlansToShow[supremeRoomPlans[index][jindex].name].push(
          supremeRoomPlans[index][jindex]
        );
      } else {
        supremePlansToShow[supremeRoomPlans[index][jindex].name].push(
          supremeRoomPlans[index][jindex]
  ...
  for (let index = 0; index < classicRoomPlans.length; index++) {
    ...
    for (let jindex = 0; jindex < classicRoomPlans[index].length; jindex++) {
      if (!classicPlansToShow[classicRoomPlans[index][jindex].name]) {
        ...
        classicPlansToShow[classicRoomPlans[index][jindex].name].push(
          classicRoomPlans[index][jindex]
        );
      } else {
        classicPlansToShow[classicRoomPlans[index][jindex].name].push(
          classicRoomPlans[index][jindex]
        ...
        <div className={styles.topBar}>
          <div className={styles.topBarButtons}>
            <div className={styles.buttonsWrapper}>
              ...
              <span>
                <GrRotateLeft size={13} style={{ marginRight: "5px" }} />
                <span>Reset</span>
              </span>
          ...
          <Row className={styles.dateSection}>
            <Col className={styles.dateSelector}>
              <span>
                ...
                />
                <AiOutlineDoubleLeft
                  onClick={fourteenDaysDecrement}
                  size={15}
                  style={{ marginRight: "10px" }}
                />
                <MdKeyboardArrowLeft onClick={sevenDaysDecrement} />
                {date.toDateString()}
                <MdKeyboardArrowRight onClick={sevenDaysIncrement} />
                <AiOutlineDoubleRight
                  onClick={fourteenDaysIncrement}
                  size={15}
                ...
                {datesToShow.map((val, i) => {
                  return (
                    <Col key={i} className={styles.dateCard}>
                      ...
                      <span className={styles.boldDateText}>{val}</span>
            ...
            DeluxeRoom.map((val,i)=>{
              ...
              return (<Col key={i} className={styles.col} >
                {val.roomDetails.length}
                    ...
                    {val.name == Object.keys(roomsNamesWithData)[index]
                      ? Object.values(roomsNamesWithData)[index].map(
                          (val, i) => {
                            return (
              ...
              <Row className={styles.content}>
                ...
                {val.name == "Deluxe Room"
                      ...
                      return (
                        <>
                          ...
                          <Col className={styles.rightSection}>
                            <Row className={styles.data}>
                              {val.name == "Deluxe Room"
                                ? Object.values(deluxPlansToShow)[i].map(
                                    (val2, j) => {
                                      return (
                                ...
                                : val.name == "Supreme Room"
                                ? Object.values(supremePlansToShow)[i].map(
                                    (val2, j) => {
                                      return (
                                        <Col key={j} className={styles.col}>
                                          {val2.amount}
                                ...
                                : val.name == "Classic Room"
                                ? Object.values(classicPlansToShow)[i].map(
                                    (val2, j) => {
                                      return (
                                        <Col key={j} className={styles.col}>
                                          {val2.amount}
                  ...
                  : val.name == "Supreme Room"
                  ? Object.keys(supremePlansToShow).map((val1, i) => {
                      return (
                        <>
                          <Col className={styles.Icon}></Col>
                          <Col className={styles.leftSection}>
                  ...
                  : val.name == "Classic Room"
                  ? Object.keys(classicPlansToShow).map((val1, i) => {
                      return (
                        <>
                          <Col className={styles.Icon}></Col>
                          <Col className={styles.leftSection}>
...
export async function getServerSideProps(context) {
  const propertiesResponse = await fetch(
    "https://api.bookonelocal.in/api-bookone/api/property/495/rooms",
    {
      ...
      headers: {
        ...
        Authorization:
          "Bearer eyJhbGciOiJIUzI1NiJ9.eyJzdWIiOiJtYWhhYmlyLmdhbGF4eUBnbWFpbC5jb20iLCJzY29wZXMiOiJST0xFX1BST1BfQURNSU4iLCJpYXQiOjE2NTY5OTI5MjksImV4cCI6MTY1NzQyNDkyOX0.kmoXtkq3kmuHaiFFzgpB23FVGMu1qZDEgIwc3-VCAc4",
  ...
  const propertyResponse = await fetch(
    "https://api.bookonelocal.in/api-bookone/api/availability/getNext7daysRatesAndAvailabilityForProperty?PropertyId=495",
    {
      method: "GET",
      headers: {
        Accept: "application/json",
  ...
  for (let index = 0; index < properties.length; index++) {
    const roomsResponse = await fetch(
      `https://api.bookonelocal.in/api-bookone/api/availability/getNext7daysRatesAndAvailabilityForRoom?PropertyId=495&RoomId=${properties[index].id}`,
      {
        method: "GET",
        headers: {
          Accept: "application/json",
